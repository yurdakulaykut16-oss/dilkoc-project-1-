...
export default function App() {
  ...
  const [screen, setScreen] = useState<'MAP' | 'ALPHA' | 'ALPHA_CHECK' | 'ALPHA_READING' | 'STORY' | 'DIALOG' | 'SMESHARIKI' | 'FLASHCARD' | 'MATCH' | 'TYPING' | 'SENTENCE' | 'QUIZ' | 'UNIT_STORY' | 'STORY_TEST' | 'STORY_RESULT'>('MAP');
  ...
  // OTOMATİK KAYIT
  useEffect(() => {
    const d: SaveState = { xp, streak, gems, completedAlpha, completedUnits, mistakes, srsBank };
    localStorage.setItem(SAVE_KEY, JSON.stringify(d));
  }, [xp, streak, gems, completedAlpha, completedUnits, mistakes, srsBank]);
 ...
 const speak = async (txt: string) => {
  try {
    ...
    // Tarayıcıda çalışırken eski yönteme düş
  ...
  // İLERLEME HESAPLAMA
  const totalTasks = ALPHABET_LESSONS.length + UNITS_DATA.length;
  const completedCount = completedAlpha.length + completedUnits.length;
      ...
      <div style={containerStyle}>
        ...
        {screen === 'MAP' && activeTab === 'MAP' && (
          <div>
            ...
            {/* AŞAMA 2: DUOLINGO TARZI BAŞLIKLI ÜNİTELER */}
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                <span style={{ background: '#f59e0b', color: '#fff', fontSize: '11px', fontWeight: 900, padding: '3px 8px', borderRadius: '6px' }}>AŞAMA 2</span>
                <h3 style={{ margin: 0, fontSize: '18px' }}>Günlük Hayat & Dizi Sahneleri Müfredatı (15 Ünite)</h3>
              </div>
